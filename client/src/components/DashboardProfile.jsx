  ...
  useEffect(() => {
    if (imageFile) {
  	...
  	const uploadImage = async () => {
		...
		uploadTask.on(
			...
			(error) => {
				setImageFileUploadError('Could not upload mage (File must be less than 2MB)');
	...
	const handleSubmit = async (e) => {
		...
		if(imageFileUploading){
			setUpdateUserError('Please wait for image to upload successfully')
			return;
		}
		try {
